...
- Suite: **sanity**
- Script ID（可能因 delete/import 變動）：`11F_131=59` / `11F_140(current)=5207`
- 風險等級（對 DUT 影響）：**HIGH**
...
env:
  ...
  # =========================
  # [Warehouse Page - IPv6 LLA]
  # - WAREHOUSE_IPV6_LL 可留空，腳本會透過 cpe_ssh.py --cmd lla 自動偵測
  # - LAN_SCOPE_IFACE：控制機對 LAN 的介面名稱（URL 必須帶 %25<iface>；若留空則用 PING_IFACE）
  # =========================
  WAREHOUSE_IPV6_LL: ''
  LLA_IFACE: 'br-home'
  LAN_SCOPE_IFACE: ''
  ...
  # REQUIRED_FIELDS：建議只放「跨環境都應該存在」的欄位。
  # - TEST_PROFILE=lab：通常沒有 WAN/global IPv6，warehouse 可能不會輸出 IPv6 相關欄位。
  # - TEST_PROFILE=customer：程式會自動要求 IPv6 Address（見 main_impl_orig.py）。
  ...
  # 若客戶環境 FR 後 sshd 起得較慢，可開啟以下等待 (預設程式也會開啟)
  ...
  # LLA 偵測若遇到 SSH timeout/port 22 尚未開，會自動 re-enable SSH via NOC 後重試
  LLA_DETECT_RETRIES: '3'
  LLA_SSH_READY_WAIT_MAX_SEC: '300'
  LLA_SSH_READY_WAIT_INTERVAL_SEC: '5'

  # 使用 cpe_ssh.py --cmd macs 取得各介面 MAC（避免解析 ifconfig -a 造成不一致）
